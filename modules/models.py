...
def Backbone(backbone_type="ResNet50", use_pretrain=True, post_name="_extractor"):
    ...
    def backbone(x):
        if backbone_type.lower() == "MLSD".lower():
            ...
            raise NotImplementedError("Backbone type {} is not supported.".format(backbone_type))
        ...
        return Model(extractor.input, output_layers, name=backbone_type + post_name)(preprocess(x))
...
class Upblock(tf.keras.layers.Layer):
    ...
    def __init__(self, out_ch, squeeze_rate, cfg, last=False):
        ...
        self.conv_block1 = Conv_BN_Act(
            out_ch // squeeze_rate, k_size=cfg.type_a_ksize, strides=1, wd=cfg.wd, act="relu"
        )
        self.conv_block2 = Conv_BN_Act(
            out_ch // squeeze_rate, k_size=cfg.type_a_ksize, strides=1, wd=cfg.wd, act="relu"
        )
        self.last = last
